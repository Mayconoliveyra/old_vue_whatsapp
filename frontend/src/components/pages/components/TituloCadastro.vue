<template>
  <div class="main-titulo-cadastro">
    <div class="titulo-cadastro">
      ...
      <button
        class="salvar"
        v-if="exibirBtnCadastro && mode != 'save' && permissaoCadastrar"
        @click="$emit('criarNovoCadastro')"
      >Adicionar</button>
      <div v-if="mode == 'save'">
        <button
          v-if="codigo && permissaoAlterar && permissaoExcluir && !api"
          class="desativar"
          @click="$emit('excluirRegistro')"
        >Excluir</button>
        <button class="cancelar" @click="$emit('cancelarCadastro')">Cancelar</button>
        <button
          v-if="codigo && permissaoAlterar"
          class="salvar"
          @click="$emit('salvarRegistro')"
        >Atualizar</button>
        <button
          v-if="!codigo && permissaoCadastrar"
          class="salvar"
          @click="$emit('salvarRegistro')"
        >Cadastrar</button>
      </div>
